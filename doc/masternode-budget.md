...
Advantage now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.

Budgets go through a series of stages before being paid:
* prepare - create a special transaction that destroys credits in order to make a proposal
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<advantage_address\> \<monthly_payment_advantage\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ Advantage, _12_ times over the course of a year totaling _24000_ Advantage.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<advantage_address\> \<monthly_payment_advantage\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` Advantage.
 ...
 * vote-alias         - Vote on a Advantage initiative
 * vote               - Vote on a Advantage initiative/budget
