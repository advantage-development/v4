...
*Setup instructions for a gitian build of Advantage using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Advantage
...
- [Building Advantage](#building-advantage)
...
Clone the git repositories for advantage and gitian and then checkout the advantage version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/advantage-crypto/advantage.git
cd advantage
...
Follow the instructions in [doc/release-process.md](release-process.md) in the advantage repository
...
To build Advantage (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the advantage repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/advantage/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/advantage-crypto/advantage
...
For example:
```bash
URL=https://github.com/crowning-/advantage.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit advantage=${COMMIT} --url advantage=${URL} ../Advantage/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit advantage=${COMMIT} --url advantage=${URL} ../Advantage/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit advantage=${COMMIT} --url advantage=${URL} ../Advantage/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/advantage-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/advantage-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/advantage-build.assert
```
...
[advantage/gitian.sigs](https://github.com/advantage-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
