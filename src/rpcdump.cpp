...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"advantageprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"advantageprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
Value importwallet(const Array& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CBitcreditAddress(keyid).ToString());
            continue;
        }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"advantageaddress\"   (string, required) The advantage address for the private key\n"
            "\nResult:\n"
    ...
    return CBitcreditSecret(vchSecret).ToString();
}
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CBitcreditAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            if (pwalletMain->mapAddressBook.count(keyid)) {
                file << strprintf("%s %s label=%s # addr=%s\n", CBitcreditSecret(key).ToString(), strTime, EncodeDumpString(pwalletMain->mapAddressBook[keyid].name), strAddr);
            } else if (setKeyPool.count(keyid)) {
                file << strprintf("%s %s reserve=1 # addr=%s\n", CBitcreditSecret(key).ToString(), strTime, strAddr);
            } else {
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"advantageaddress\"\n"
            "\nEncrypts a private key corresponding to 'advantageaddress'.\n"
            "\nArguments:\n"
            "1. \"advantageaddress\"   (string, required) The advantage address for the private key (you must hold the key already)\n"
...
Value bip38decrypt(const Array& params, bool fHelp)
{
    ...
    pubkey.IsCompressed();
    assert(key.VerifyPubKey(pubkey));
    result.push_back(Pair("Address", CBitcreditAddress(pubkey.GetID()).ToString()));
    CKeyID vchAddress = pubkey.GetID();
    {
        pwalletMain->MarkDirty();
