...
//
void CObfuscationPool::UnlockCredits()
{
    while (true) {
        ...
        BOOST_FOREACH (CTxIn v, lockedCredits)
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (!pwalletMain->SelectCreditsDark(nValueMin, nBalanceNeedsAnonymized, vCredits, nValueIn, 0, nObfuscationRounds)) {
        nValueIn = 0;
        vCredits.clear();

        if (pwalletMain->SelectCreditsDark(nValueMin, 9999999 * CREDIT, vCredits, nValueIn, -2, 0)) {
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                std::vector<CTxIn> vTempCredits;
                std::vector<COutput> vTempCredits2;
                // Try to match their denominations if possible
                if (!pwalletMain->SelectCreditsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCredits, vTempCredits2, nValueIn, 0, nObfuscationRounds)) {
        ...
        while (i < 10) {
            ...
            if (pnode != NULL) {
                ...
                pwalletMain->ConvertList(vCredits, vecAmounts);
