...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    pwalletMain->AvailableCredits(vecOutputs, false);
...
Value signrawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        LOCK(mempool.cs);
        CCreditsViewCache& viewChain = *pcreditsTip;
        CCreditsViewMemPool viewMempool(&viewChain, mempool);
        view.SetBackend(viewMempool); // temporarily switch cache backend to db+mempool view

        BOOST_FOREACH (const CTxIn& txin, mergedTx.vin) {
            const uint256& prevHash = txin.prevout.hash;
            CCredits credits;
            view.AccessCredits(prevHash); // this is certainly allowed to fail
        }
...
Value sendrawtransaction(const Array& params, bool fHelp)
{
    ...
    CCreditsViewCache& view = *pcreditsTip;
    const CCredits* existingCredits = view.AccessCredits(hashTx);
    bool fHaveMempool = mempool.exists(hashTx);
    bool fHaveChain = existingCredits && existingCredits->nHeight < 1000000000;
