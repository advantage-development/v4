...
CAmount WalletModel::getBalance(const CCoinControl* creditControl) const
{
    if (creditControl) {
        ...
        wallet->AvailableCredits(vCredits, true, creditControl);
...
WalletModel::SendCreditsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* creditControl)
{
    CAmount total = 0;
    QList<SendCreditsRecipient> recipients = transaction.getRecipients();
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * CREDIT) {
            emit message(tr("Send Credits"), tr("SwiftTX doesn't support sending values that high yet. Transactions are currently limited to %1 USDA.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
        ...
        if (!fCreated) {
            if ((total + nFeeRequired) > nBalance) {
                return SendCreditsReturn(AmountWithFeeExceedsBalance);
            }
            emit message(tr("Send Credits"), QString::fromStdString(strFailReason),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
    ...
    return SendCreditsReturn(OK);
}

WalletModel::SendCreditsReturn WalletModel::sendCredits(WalletModelTransaction& transaction)
{
    ...
    foreach (const SendCreditsRecipient& rcp, transaction.getRecipients()) {
...
// AvailableCredits + LockedCredits grouped by wallet address (put change in one group with wallet address)
void WalletModel::listCredits(std::map<QString, std::vector<COutput> >& mapCredits) const
{
    std::vector<COutput> vCredits;
    wallet->AvailableCredits(vCredits);

    LOCK2(cs_main, wallet->cs_wallet); // ListLockedCredits, mapWallet
    std::vector<COutPoint> vLockedCredits;
    wallet->ListLockedCredits(vLockedCredits);

    // add locked credits
    BOOST_FOREACH (const COutPoint& outpoint, vLockedCredits) {
        if (!wallet->mapWallet.count(outpoint.hash)) continue;
        int nDepth = wallet->mapWallet[outpoint.hash].GetDepthInMainChain();
        if (nDepth < 0) continue;
        COutput out(&wallet->mapWallet[outpoint.hash], outpoint.n, nDepth, true);
        if (outpoint.n < out.tx->vout.size() && wallet->IsMine(out.tx->vout[outpoint.n]) == ISMINE_SPENDABLE)
            vCredits.push_back(out);
    }
    ...
    BOOST_FOREACH (const COutput& out, vCredits) {
        ...
        if (!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCredits[QString::fromStdString(CBitcreditAddress(address).ToString())].push_back(out);
    }
}
...
void WalletModel::listLockedCredits(std::vector<COutPoint>& vOutpts)
{
    LOCK2(cs_main, wallet->cs_wallet);
    wallet->ListLockedCredits(vOutpts);
}
