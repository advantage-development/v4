...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcreditUnits::formatWithUnit(BitcreditUnits::USDA, nValue);
...
std::string AddressToString(const CBitcreditAddress& Address)
{
    std::string TxLabels[] =
        {
            _("Date"),
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (advantage.conf).");
            QMessageBox::warning(this, "Advantage Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
