                                          ...
                                          walletModel(0)
{
    ...
    receiveCreditsPage = new ReceiveCreditsDialog();
    sendCreditsPage = new SendCreditsDialog();

    addWidget(overviewPage);
    addWidget(transactionsPage);
    addWidget(receiveCreditsPage);
    addWidget(sendCreditsPage);
    ...
    // Pass through messages from sendCreditsPage
    connect(sendCreditsPage, SIGNAL(message(QString, QString, unsigned int)), this, SIGNAL(message(QString, QString, unsigned int)));
...
void WalletView::setClientModel(ClientModel* clientModel)
{
    ...
    sendCreditsPage->setClientModel(clientModel);
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void WalletView::setWalletModel(WalletModel* walletModel)
{
    ...
    receiveCreditsPage->setModel(walletModel);
    sendCreditsPage->setModel(walletModel);
...
void WalletView::gotoReceiveCreditsPage()
{
    setCurrentWidget(receiveCreditsPage);
}

void WalletView::gotoSendCreditsPage(QString addr)
{
    setCurrentWidget(sendCreditsPage);

    if (!addr.isEmpty())
        sendCreditsPage->setAddress(addr);
}
...
bool WalletView::handlePaymentRequest(const SendCreditsRecipient& recipient)
{
    return sendCreditsPage->handlePaymentRequest(recipient);
}
