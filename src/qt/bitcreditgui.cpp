...
void BitcreditGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCreditsAction = new QAction(QIcon(":/icons/send"), tr("&Send"), this);
    sendCreditsAction->setStatusTip(tr("Send credits to a Advantage address"));
    sendCreditsAction->setToolTip(sendCreditsAction->statusTip());
    sendCreditsAction->setCheckable(true);
#ifdef Q_OS_MAC
    sendCreditsAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_2));
#else
    sendCreditsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_2));
#endif
    tabGroup->addAction(sendCreditsAction);

    receiveCreditsAction = new QAction(QIcon(":/icons/receiving_addresses"), tr("&Receive"), this);
    receiveCreditsAction->setStatusTip(tr("Request payments (generates QR codes and advantage: URIs)"));
    receiveCreditsAction->setToolTip(receiveCreditsAction->statusTip());
    receiveCreditsAction->setCheckable(true);
#ifdef Q_OS_MAC
    receiveCreditsAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_3));
#else
    receiveCreditsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_3));
#endif
    tabGroup->addAction(receiveCreditsAction);
    ...
    // These showNormalIfMinimized are needed because Send Credits and Receive Credits
    // can be triggered from the tray menu, and need to show the GUI to be useful.
    connect(overviewAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(overviewAction, SIGNAL(triggered()), this, SLOT(gotoOverviewPage()));
    connect(sendCreditsAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(sendCreditsAction, SIGNAL(triggered()), this, SLOT(gotoSendCreditsPage()));
    connect(receiveCreditsAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(receiveCreditsAction, SIGNAL(triggered()), this, SLOT(gotoReceiveCreditsPage()));
...
void BitcreditGUI::createToolBars()
{
    if (walletFrame) {
        ...
        toolbar->addAction(sendCreditsAction);
        toolbar->addAction(receiveCreditsAction);
...
void BitcreditGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    sendCreditsAction->setEnabled(enabled);
    receiveCreditsAction->setEnabled(enabled);
...
void BitcreditGUI::createTrayIconMenu()
{
    ...
    trayIconMenu->addAction(sendCreditsAction);
    trayIconMenu->addAction(receiveCreditsAction);
    trayIconMenu->addSeparator();
...
void BitcreditGUI::gotoReceiveCreditsPage()
{
    receiveCreditsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCreditsPage();
}

void BitcreditGUI::gotoSendCreditsPage(QString addr)
{
    sendCreditsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCreditsPage(addr);
}
...
#ifdef ENABLE_WALLET
bool BitcreditGUI::handlePaymentRequest(const SendCreditsRecipient& recipient)
{
    // URI has to be valid
