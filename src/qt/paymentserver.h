...
// but the main window (and the Send Credits tab) is not.
//
// So, the strategy is:
//
...
signals:
    ...
    void receivedPaymentRequest(SendCreditsRecipient);
...
public slots:
    ...
    void fetchPaymentACK(CWallet* wallet, SendCreditsRecipient recipient, QByteArray transaction);
...
private:
    bool processPaymentRequest(PaymentRequestPlus& request, SendCreditsRecipient& recipient);
