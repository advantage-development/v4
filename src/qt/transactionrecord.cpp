...
{
    if (wtx.IsCoinBase()) {
        // Ensures we show generated credits / mined transactions at depth 1
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (!IsMine(*wallet, address)) {
            //if the address is not yours then it means you have a tx sent to you in someone elses creditstake tx
            for (unsigned int i = 1; i < wtx.vout.size(); i++) {
                ...
                if (ExtractDestination(wtx.vout[i].scriptPubKey, outAddress)) {
                    if (IsMine(*wallet, outAddress)) {
                        ...
                        sub.address = CBitcreditAddress(outAddress).ToString();
