...
public:
    explicit AmountSpinBox(QWidget* parent) : QAbstractSpinBox(parent),
                                              currentUnit(BitcreditUnits::USDA),
    ...
    void fixup(QString& input) const
    {
        ...
        if (valid) {
            input = BitcreditUnits::format(currentUnit, val, false, BitcreditUnits::separatorAlways);
    ...
    {
        lineEdit()->setText(BitcreditUnits::format(currentUnit, value, false, BitcreditUnits::separatorAlways));
        emit valueChanged();
    }
    ...
    void stepBy(int steps)
    {
        ...
        val = qMin(qMax(val, CAmount(0)), BitcreditUnits::maxMoney());
        setValue(val);
    }
    ...
    QSize minimumSizeHint() const
    {
        if (cachedMinimumSizeHint.isEmpty()) {
            ...
            int w = fm.width(BitcreditUnits::format(BitcreditUnits::USDA, BitcreditUnits::maxMoney(), false, BitcreditUnits::separatorAlways));
...
private:
    ...
    {
        CAmount val = 0;
        bool valid = BitcreditUnits::parse(currentUnit, text, &val);
        if (valid) {
            if (val < 0 || val > BitcreditUnits::maxMoney())
                valid = false;
        }
        if (valid_out)
            *valid_out = valid;
...
#include "bitcreditamountfield.moc"

BitcreditAmountField::BitcreditAmountField(QWidget* parent) : QWidget(parent),
                                                          amount(0)
{
...
void BitcreditAmountField::setEnabled(bool fEnabled)
{
...
CAmount BitcreditAmountField::value(bool* valid_out) const
{
    return amount->value(valid_out);
}

void BitcreditAmountField::setValue(const CAmount& value)
{
...
void BitcreditAmountField::setSingleStep(const CAmount& step)
{
