...
std::string CTxIn::ToString() const
{
    ...
    if (prevout.IsNull())
        str += strprintf(", creditbase %s", HexStr(scriptSig));
    else
...
{
    return strprintf("CTxOut(nValue=%d.%08d, scriptPubKey=%s)", nValue / CREDIT, nValue % CREDIT, scriptPubKey.ToString().substr(0,30));
}
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // Advantage: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
