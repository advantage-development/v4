...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
        {
            ...
            bool sigOK = CScriptCheck(CCredits(txFrom, 0), txTo[i], 0, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, false)();
            if (i == j)
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    credits.ModifyCredits(txFrom.GetHash())->FromTx(txFrom, 0);
