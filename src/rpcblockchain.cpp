...
Value gettxout(const Array& params, bool fHelp)
{
    ...
    if (fMempool) {
        LOCK(mempool.cs);
        CCreditsViewMemPool view(pcreditsTip, mempool);
        if (!view.GetCredits(hash, credits))
            return Value::null;
        mempool.pruneSpent(hash, credits); // TODO: this should be done by the CCreditsViewMemPool
    } else {
        if (!pcreditsTip->GetCredits(hash, credits))
            return Value::null;
    }
