...
static const CAmount nHighTransactionFeeWarning = 0.1 * CREDIT;
//! -maxtxfee default
static const CAmount DEFAULT_TRANSACTION_MAXFEE = 1 * CREDIT;
...
{
private:
    bool SelectCredits(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& setCreditsRet, CAmount& nValueRet, const CCoinControl* coinControl = NULL, AvailableCreditsType coin_type = ALL_CREDITS, bool useIX = true) const;
    //it was public bool SelectCredits(int64_t nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCreditsRet, int64_t& nValueRet, const CCoinControl *coinControl = NULL, AvailableCreditsType coin_type=ALL_CREDITS, bool useIX = true) const;
...
public:
    bool MintableCredits();
    bool SelectStakeCredits(std::set<std::pair<const CWalletTx*, unsigned int> >& setCredits, CAmount nTargetAmount) const;
    bool SelectCreditsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCreditsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const;
    bool SelectCreditsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCreditsRet, std::vector<COutput>& vCreditsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax);
    bool SelectCreditsDarkDenominated(CAmount nTargetValue, std::vector<CTxIn>& setCreditsRet, CAmount& nValueRet) const;
    bool HasCollateralInputs(bool fOnlyConfirmed = true) const;
    bool IsCollateralAmount(CAmount nInputAmount) const;
    int CountInputsWithAmount(CAmount nInputAmount);

    bool SelectCreditsCollateral(std::vector<CTxIn>& setCreditsRet, CAmount& nValueRet) const;
    ...
    std::set<COutPoint> setLockedCredits;
    ...
    void AvailableCredits(std::vector<COutput>& vCredits, bool fOnlyConfirmed = true, const CCoinControl* coinControl = NULL, bool fIncludeZeroValue = false, AvailableCreditsType nCoinType = ALL_CREDITS, bool fUseIX = false) const;
    std::map<CBitcreditAddress, std::vector<COutput> > AvailableCreditsByAddress(bool fConfirmed = true, CAmount maxCoinValue = 0);
    bool SelectCreditsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, std::vector<COutput> vCredits, std::set<std::pair<const CWalletTx*, unsigned int> >& setCreditsRet, CAmount& nValueRet) const;
    ...
    void UnlockAllCredits();
    void ListLockedCredits(std::vector<COutPoint>& vOutpts);
    CAmount GetTotalValue(std::vector<CTxIn> vCredits);
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, CAmount> >& vecSend,
        ...
        AvailableCreditsType coin_type = ALL_CREDITS,
        bool useIX = false,
        CAmount nFeePay = 0);
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl = NULL, AvailableCreditsType coin_type = ALL_CREDITS, bool useIX = false, CAmount nFeePay = 0);
    ...
    bool ConvertList(std::vector<CTxIn> vCredits, std::vector<int64_t>& vecAmounts);
