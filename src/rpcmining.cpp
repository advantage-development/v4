...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or advantage.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    } else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateBitcredits(fGenerate, pwalletMain, nGenProcLimit);
    }
...
Value prioritisetransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "                  (priority of a transaction is calculated: creditage * value_in_duffs / txsize) \n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.bitcredit.it/wiki/BIP_0022 for full specification.\n"
            ...
            "           \"support\"           (string) client side supported feature, 'longpoll', 'creditbasetxn', 'creditbasevalue', 'proposal', 'serverlist', 'workid'\n"
            "           ,...\n"
            "         ]\n"
            "     }\n"
            "\n"
            ...
            "  \"transactions\" : [                (array) contents of non-creditbase transactions that should be included in the next block\n"
            "      {\n"
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                   (numeric) difference in value between transaction inputs and outputs (in duffs); for creditbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            "         \"sigops\" : n,               (numeric) total number of SigOps, as counted for purposes of block limits; if key is not present, sigop count is unknown and clients MUST NOT assume there aren't any\n"
            "         \"required\" : true|false     (boolean) if provided and true, this transaction must be in the final block\n"
            "      }\n"
            "      ,...\n"
            "  ],\n"
            "  \"creditbaseaux\" : {                  (json object) data that should be included in the creditbase's scriptSig content\n"
            "      \"flags\" : \"flags\"            (string) \n"
            "  },\n"
            "  \"creditbasevalue\" : n,               (numeric) maximum allowable input to creditbase transaction, including the generation award and transaction fees (in duffs)\n"
            "  \"creditbasetxn\" : { ... },           (json object) information for creditbase transaction\n"
...
// Release the wallet and main lock while waiting
    ...
    result.push_back(Pair("creditbaseaux", aux));
    result.push_back(Pair("creditbasevalue", (int64_t)pblock->vtx[0].GetValueOut()));
