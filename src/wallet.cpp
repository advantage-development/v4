...
// Copyright (c) 2018 The AdvantageCoin developers
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    std::vector<COutput> vPossibleCredits;
    AvailableCredits(vPossibleCredits, true, NULL, false, ONLY_10000);
    if (vPossibleCredits.empty()) {
        LogPrintf("CWallet::GetMasternodeVinAndKeys -- Could not locate any valid masternode vin\n");
        return false;
    }

    if (strTxHash.empty()) // No output specified, select the first one
        return GetVinAndKeysFromOutput(vPossibleCredits[0], txinRet, pubKeyRet, keyRet);
    ...
    BOOST_FOREACH (COutput& out, vPossibleCredits)
...
/**
 * populate vCredits with vector of available COutputs.
 */
void CWallet::AvailableCredits(vector<COutput>& vCredits, bool fOnlyConfirmed, const CCoinControl* coinControl, bool fIncludeZeroValue, AvailableCreditsType nCoinType, bool fUseIX) const
{
    vCredits.clear();
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterNode && pcoin->vout[i].nValue == Params().MasternodeCollateralLimit() * CREDIT);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT10000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fMasterNode) found = pcoin->vout[i].nValue != Params().MasternodeCollateralLimit() * CREDIT; // do not use Hot MN funds
                } else if (nCoinType == ONLY_10000) {
                    found = pcoin->vout[i].nValue == Params().MasternodeCollateralLimit() * CREDIT;
                } else {
                    found = true;
                }
                if (!found) continue;
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                    ...
                    vCredits.push_back(COutput(pcoin, i, nDepth,
...
map<CBitcreditAddress, vector<COutput> > CWallet::AvailableCreditsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    vector<COutput> vCredits;
    AvailableCredits(vCredits, fConfirmed);

    map<CBitcreditAddress, vector<COutput> > mapCredits;
    BOOST_FOREACH (COutput out, vCredits) {
        ...
        mapCredits[CBitcreditAddress(address)].push_back(out);
    }
...
bool CWallet::SelectStakeCredits(std::set<std::pair<const CWalletTx*, unsigned int> >& setCredits, CAmount nTargetAmount) const
{
    vector<COutput> vCredits;
    AvailableCredits(vCredits, true);
    CAmount nAmountSelected = 0;

    BOOST_FOREACH (const COutput& out, vCredits) {
        ...
        setCredits.insert(make_pair(out.tx, out.i));
        nAmountSelected += out.tx->vout[out.i].nValue;
    }
    return true;
}

bool CWallet::MintableCredits()
{
    CAmount nBalance = GetBalance();
    if (mapArgs.count("-reservebalance") && !ParseMoney(mapArgs["-reservebalance"], nReserveBalance))
        return error("MintableCredits() : invalid reserve balance amount");
...
bool CWallet::SelectCreditsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCredits, set<pair<const CWalletTx*, unsigned int> >& setCreditsRet, CAmount& nValueRet) const
{
    setCreditsRet.clear();
    nValueRet = 0;
    ...
    random_shuffle(vCredits.begin(), vCredits.end(), GetRandInt);

    // move denoms down on the list
    sort(vCredits.begin(), vCredits.end(), less_then_denom);
    ...
    for (unsigned int tryDenom = 0; tryDenom < 2; tryDenom++) {
        ...
        vValue.clear();
        nTotalLower = 0;
        BOOST_FOREACH (const COutput& output, vCredits) {
            ...
            if (n == nTargetValue) {
                setCreditsRet.insert(coin.second);
        ...
        if (nTotalLower == nTargetValue) {
            for (unsigned int i = 0; i < vValue.size(); ++i) {
                setCreditsRet.insert(vValue[i].second);
        ...
        if (nTotalLower < nTargetValue) {
            ...
            setCreditsRet.insert(coinLowestLarger.second);
    ...
    if (coinLowestLarger.second.first &&
        ...
        string s = "CWallet::SelectCreditsMinConf best subset: ";
        for (unsigned int i = 0; i < vValue.size(); i++) {
            if (vfBest[i]) {
                setCreditsRet.insert(vValue[i].second);
                nValueRet += vValue[i].first;
...
bool CWallet::SelectCredits(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCreditsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCreditsType coin_type, bool useIX) const
{
    // Note: this function should never be used for "always free" tx types like dstx

    vector<COutput> vCredits;
    AvailableCredits(vCredits, true, coinControl, false, coin_type, useIX);
    ...
    if (coinControl && coinControl->HasSelected()) {
        BOOST_FOREACH (const COutput& out, vCredits) {
            ...
            setCreditsRet.insert(make_pair(out.tx, out.i));
        }
    ...
    return (SelectCreditsMinConf(nTargetValue, 1, 6, vCredits, setCreditsRet, nValueRet) ||
            SelectCreditsMinConf(nTargetValue, 1, 1, vCredits, setCreditsRet, nValueRet) ||
            (bSpendZeroConfChange && SelectCreditsMinConf(nTargetValue, 0, 1, vCredits, setCreditsRet, nValueRet)));
}
...
bool CWallet::SelectCreditsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCreditsRet, std::vector<COutput>& vCreditsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    vCreditsRet.clear();
    nValueRet = 0;
    ...
    vCreditsRet2.clear();
    vector<COutput> vCredits;
    AvailableCredits(vCredits, true, NULL, ONLY_DENOMINATED);

    std::random_shuffle(vCredits.rbegin(), vCredits.rend());
    ...
    BOOST_FOREACH (const COutput& out, vCredits) {
        // masternode-like input should not be selected by AvailableCredits now anyway
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            if (fFound10000 && fFound1000 && fFound100 && fFound10 && fFound1 && fFoundDot1) { //if fulfilled
                ...
                if (nValueRet >= nValueMin) {
                    ...
                    int r = (rand() % (int)vCredits.size());
                    if ((int)vCreditsRet.size() > r) return true;
                }
            ...
            nValueRet += out.tx->vout[out.i].nValue;
            vCreditsRet.push_back(vin);
            vCreditsRet2.push_back(out);
        }
    }
...
bool CWallet::SelectCreditsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCreditsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    vector<COutput> vCredits;
    AvailableCredits(vCredits, true, coinControl, nObfuscationRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);

    set<pair<const CWalletTx*, unsigned int> > setCreditsRet2;

    //order the array so largest nondenom are first, then denominations, then very small inputs.
    sort(vCredits.rbegin(), vCredits.rend(), CompareByPriority());
    ...
    BOOST_FOREACH (const COutput& out, vCredits) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            vin.prevPubKey = out.tx->vout[out.i].scriptPubKey; // the inputs PubKey
            nValueRet += out.tx->vout[out.i].nValue;
            setCreditsRet.push_back(vin);
            setCreditsRet2.insert(make_pair(out.tx, out.i));
        }
    }
...
bool CWallet::SelectCreditsCollateral(std::vector<CTxIn>& setCreditsRet, CAmount& nValueRet) const
{
    vector<COutput> vCredits;

    //LogPrintf(" selecting credits for collateral\n");
    AvailableCredits(vCredits);
...
{
    vector<COutput> vCredits;
    AvailableCredits(vCredits, fOnlyConfirmed);
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    CAmount nValueIn2 = 0;
    std::vector<CTxIn> vCreditsCollateral;

    if (!SelectCreditsCollateral(vCreditsCollateral, nValueIn2)) {
    ...
    BOOST_FOREACH (CTxIn v, vCreditsCollateral)
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    CCoinControl* coinControl = NULL;
    bool success = CreateTransaction(vecSend, tx, reservekey, nFeeRet, strFail, coinControl, ALL_CREDITS, useIX, (CAmount)0);
    if (!success) {
...
bool CWallet::ConvertList(std::vector<CTxIn> vCredits, std::vector<CAmount>& vecAmounts)
{
    BOOST_FOREACH (CTxIn i, vCredits) {
...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecSend,
    ...
    AvailableCreditsType coin_type,
    bool useIX,
    CAmount nFeePay)
{
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                set<pair<const CWalletTx*, unsigned int> > setCredits;
                CAmount nValueIn = 0;

                if (!SelectCredits(nTotalValue, setCredits, nValueIn, coinControl, coin_type, useIX)) {
                ...
                BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCredits) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-advantagecredit-address
                    CScript scriptChange;
                ...
                // Fill vin
                BOOST_FOREACH (const PAIRTYPE(const CWalletTx*, unsigned int) & coin, setCredits)
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCreditsType coin_type, bool useIX, CAmount nFeePay)
{
    vector<pair<CScript, CAmount> > vecSend;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    static std::set<pair<const CWalletTx*, unsigned int> > setStakeCredits;
    static int nLastStakeSetUpdate = 0;

    if (GetTime() - nLastStakeSetUpdate > nStakeSetUpdateTime) {
        setStakeCredits.clear();
        if (!SelectStakeCredits(setStakeCredits, nBalance - nReserveBalance))
            return false;

        nLastStakeSetUpdate = GetTime();
    }

    if (setStakeCredits.empty())
        return false;
    ...
    BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCredits) {
	...
	int64_t treasuryPay = nCredit * 0.24;
	...
	if (pIndex0->nHeight > 1) {
		// Finally pay the 
		CBitcreditAddress address(Params().GetTreasuryRewardAddress());
		assert(address.IsValid());
		//assert(address.IsScript());
		CTxDestination dest = address.Get();
		CScript script = GetScriptForDestination(dest);
		txNew.vout.push_back(CTxOut(treasuryPay + (pIndex0->nHeight % 100 ? 0 : 1000000) , script));
		LogPrintf("CreateCoinStake : Paid TREASURY");
	}
...
CAmount CWallet::GetTotalValue(std::vector<CTxIn> vCredits)
{
...
string CWallet::PrepareObfuscationDenominate(int minRounds, int maxRounds)
{
    ...
    std::vector<CTxIn> vCredits;
    std::vector<CTxIn> vCreditsResult;
    std::vector<COutput> vCredits2;
    CAmount nValueIn = 0;
    CReserveKey reservekey(this);
    ...
    */
    if (minRounds >= 0) {
        if (!SelectCreditsByDenominations(obfuScationPool.sessionDenom, 0.1 * CREDIT, OBFUSCATION_POOL_MAX, vCredits, vCredits2, nValueIn, minRounds, maxRounds))
    ...
    {
        LOCK(cs_wallet);
        BOOST_FOREACH (CTxIn v, vCredits)
            LockCoin(v.prevout);
    }
    ...
    */
    ...
    // initially shuffled in CWallet::SelectCreditsByDenominations already.
    int nStep = 0;
    ...
    while (nStep < nStepsMax) {
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            ...
            if (nValueLeft - v >= 0) {
                ...
                std::vector<CTxIn>::iterator it = vCredits.begin();
                std::vector<COutput>::iterator it2 = vCredits2.begin();
                while (it2 != vCredits2.end()) {
                    // we have matching inputs
                    if ((*it2).tx->vout[(*it2).i].nValue == v) {
                        // add new input in resulting vector
                        vCreditsResult.push_back(*it);
    ...
    if (obfuScationPool.GetDenominations(vOut) != obfuScationPool.sessionDenom) {
        ...
        LOCK(cs_wallet);
        BOOST_FOREACH (CTxIn v, vCreditsResult)
            UnlockCoin(v.prevout);
    ...
    obfuScationPool.SendObfuscationDenominate(vCreditsResult, vOut, nValueIn - nValueLeft);
...
{
    AssertLockHeld(cs_wallet); // setLockedCredits
    setLockedCredits.insert(output);
}

void CWallet::UnlockCoin(COutPoint& output)
{
    AssertLockHeld(cs_wallet); // setLockedCredits
    setLockedCredits.erase(output);
}

void CWallet::UnlockAllCredits()
{
    AssertLockHeld(cs_wallet); // setLockedCredits
    setLockedCredits.clear();
}
...
bool CWallet::IsLockedCoin(uint256 hash, unsigned int n) const
{
    ...
    return (setLockedCredits.count(outpt) > 0);
}

void CWallet::ListLockedCredits(std::vector<COutPoint>& vOutpts)
{
    AssertLockHeld(cs_wallet); // setLockedCredits
    for (std::set<COutPoint>::iterator it = setLockedCredits.begin();
         it != setLockedCredits.end(); it++) {
        COutPoint outpt = (*it);
...
void CWallet::AutoCombineDust()
{
    ...
    map<CBitcreditAddress, vector<COutput> > mapCreditsByAddress = AvailableCreditsByAddress(true, 0);

    //credits are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (map<CBitcreditAddress, vector<COutput> >::iterator it = mapCreditsByAddress.begin(); it != mapCreditsByAddress.end(); it++) {
        vector<COutput> vCredits, vRewardCredits;
        vCredits = it->second;
        ...
        //get the fee amount
        CWalletTx wtxdummy;
        CreateTransaction(vecSend, wtxdummy, keyChange, nFeeRet, strErr, coinControl, ALL_CREDITS, false, CAmount(0));
        vecSend[0].second = nTotalRewardsValue - nFeeRet - 500;

        if (!CreateTransaction(vecSend, wtx, keyChange, nFeeRet, strErr, coinControl, ALL_CREDITS, false, CAmount(0))) {
