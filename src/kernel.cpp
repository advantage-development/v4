...
// The purpose of stake modifier is to prevent a txout (credit) owner from
...
// modifier about a selection interval later than the credit generating the kernel
...
{
    //Advantage will hash in the transaction hash and the index number in order to make sure each hash is unique
...
{
    //get the stake weight - weight is equal to credit amount
...
// Check kernel hash target and creditstake signature
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake)
{
    const CTransaction tx = block.vtx[1];
    if (!tx.IsCoinStake())
        return error("CheckProofOfStake() : called on non-creditstake %s", tx.GetHash().ToString().c_str());

    // Kernel (input 0) must match the stake hash target per credit age (nBits)
    ...
    if (!VerifyScript(txin.scriptSig, txPrev.vout[txin.prevout.n].scriptPubKey, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&tx, 0)))
        return error("CheckProofOfStake() : VerifySignature failed on creditstake %s", tx.GetHash().ToString().c_str());
    ...
    if (!CheckStakeKernelHash(block.nBits, blockprev, txPrev, txin.prevout, nTime, nInterval, true, hashProofOfStake, fDebug))
        return error("CheckProofOfStake() : INFO: check kernel failed on creditstake %s, hashProof=%s \n", tx.GetHash().ToString().c_str(), hashProofOfStake.ToString().c_str()); // may occur during initial download or if behind on block chain sync

    return true;
}

// Check whether the creditstake timestamp meets protocol
