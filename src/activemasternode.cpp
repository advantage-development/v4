...
bool CActiveMasternode::GetMasterNodeVin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex)
{
    ...
    vector<COutput> possibleCredits = SelectCreditsMasternode();
    ...
    if (!strTxHash.empty()) {
        ...
        bool found = false;
        BOOST_FOREACH (COutput& out, possibleCredits) {
        ...
        if (possibleCredits.size() > 0) {
            selectedOutput = &possibleCredits[0];
        } else {
...
vector<COutput> CActiveMasternode::SelectCreditsMasternode()
{
    vector<COutput> vCredits;
    vector<COutput> filteredCredits;
    vector<COutPoint> confLockedCredits;
    ...
    if (GetBoolArg("-mnconflock", true)) {
        ...
        BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            confLockedCredits.push_back(outpoint);
            pwalletMain->UnlockCoin(outpoint);
        }
    }

    // Retrieve all possible outputs
    pwalletMain->AvailableCredits(vCredits);

    // Lock MN credits from masternode.conf back if they where temporary unlocked
    if (!confLockedCredits.empty()) {
        BOOST_FOREACH (COutPoint outpoint, confLockedCredits)
