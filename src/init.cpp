...
class CCreditsViewErrorCatcher : public CCreditsViewBacked
{
public:
    CCreditsViewErrorCatcher(CCreditsView* view) : CCreditsViewBacked(view) {}
    bool GetCredits(const uint256& txid, CCredits& credits) const
    {
        try {
            return CCreditsViewBacked::GetCredits(txid, credits);
...
static CCreditsViewDB* pcreditsdbview = NULL;
static CCreditsViewErrorCatcher* pcreditscatcher = NULL;
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                pcreditsdbview = new CCreditsViewDB(nCoinDBCache, false, fReindex);
                pcreditscatcher = new CCreditsViewErrorCatcher(pcreditsdbview);
                pcreditsTip = new CCreditsViewCache(pcreditscatcher);
