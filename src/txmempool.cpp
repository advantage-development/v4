...
void CTxMemPool::pruneSpent(const uint256& hashTx, CCredits& credits)
{
    LOCK(cs);
...
void CTxMemPool::removeCoinbaseSpends(const CCreditsViewCache* pcredits, unsigned int nMemPoolHeight)
{
    ...
    for (std::map<uint256, CTxMemPoolEntry>::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        BOOST_FOREACH (const CTxIn& txin, tx.vin) {
            ...
            if (it2 != mapTx.end())
                continue;
            const CCredits* credits = pcredits->AccessCredits(txin.prevout.hash);
...
void CTxMemPool::check(const CCreditsViewCache* pcredits) const
{
    if (!fSanityCheck)
        return;
    ...
    CCreditsViewCache mempoolDuplicate(const_cast<CCreditsViewCache*>(pcredits));
    ...
    for (std::map<uint256, CTxMemPoolEntry>::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        if (fDependsWait)
            ...
            UpdateCredits(tx, state, mempoolDuplicate, undo, 1000000);
        }
    }
    ...
    while (!waitingOnDependants.empty()) {
        ...
        if (!mempoolDuplicate.HaveInputs(entry->GetTx())) {
            ...
            CTxUndo undo;
            UpdateCredits(entry->GetTx(), state, mempoolDuplicate, undo, 1000000);
...
CCreditsViewMemPool::CCreditsViewMemPool(CCreditsView* baseIn, CTxMemPool& mempoolIn) : CCreditsViewBacked(baseIn), mempool(mempoolIn) {}

bool CCreditsViewMemPool::GetCredits(const uint256& txid, CCredits& credits) const
{
    ...
    if (mempool.lookup(txid, tx)) {
        credits = CCredits(tx, MEMPOOL_HEIGHT);
        return true;
    }
    return (base->GetCredits(txid, credits) && !credits.IsPruned());
}

bool CCreditsViewMemPool::HaveCredits(const uint256& txid) const
{
    return mempool.exists(txid) || base->HaveCredits(txid);
}
