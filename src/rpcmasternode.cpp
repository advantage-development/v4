...
void SendMoney(const CTxDestination& address, CAmount nValue, CWalletTx& wtxNew, AvailableCreditsType credit_type = ALL_CREDITS)
{
    // Check amount
    if (nValue <= 0)
...
Value getmasternodeoutputs (const Array& params, bool fHelp)
{
    ...
    vector<COutput> possibleCredits = activeMasternode.SelectCreditsMasternode();
