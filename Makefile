...
pkgdatadir = $(datadir)/advantage
pkgincludedir = $(includedir)/advantage
pkglibdir = $(libdir)/advantage
pkglibexecdir = $(libexecdir)/advantage
...
am__aclocal_m4_deps = $(top_srcdir)/build-aux/m4/ax_boost_base.m4 \
	...
	$(top_srcdir)/build-aux/m4/bitcredit_find_bdb48.m4 \
	$(top_srcdir)/build-aux/m4/bitcredit_qt.m4 \
	$(top_srcdir)/build-aux/m4/bitcredit_subdir_to_include.m4 \
...
CONFIG_HEADER = $(top_builddir)/src/config/advantage-config.h
CONFIG_CLEAN_FILES = share/setup.nsi share/qt/Info.plist \
	src/test/buildenv.py qa/pull-tester/run-bitcreditd-for-test.sh \
...
ACLOCAL = ${SHELL} /home/advantage/advantage-core/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/advantage/advantage-core/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/advantage/advantage-core/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/advantage/advantage-core/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/advantage/advantage-core/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = advantage
PACKAGE_BUGREPORT = www.advantage.org
PACKAGE_NAME = Advantage Core
PACKAGE_STRING = Advantage Core 1.0.0
PACKAGE_TARNAME = advantage
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/advantage/advantage-core
abs_srcdir = /home/advantage/advantage-core
abs_top_builddir = /home/advantage/advantage-core
abs_top_srcdir = /home/advantage/advantage-core
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/advantage/advantage-core/build-aux/install-sh
...
SUBDIRS = src
BITCREDITD_BIN = $(top_builddir)/src/advantaged$(EXEEXT)
BITCREDIT_QT_BIN = $(top_builddir)/src/qt/advantage-qt$(EXEEXT)
BITCREDIT_CLI_BIN = $(top_builddir)/src/advantage-cli$(EXEEXT)
BITCREDIT_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Advantage-Qt.app
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/bitcredit.icns
OSX_PLIST = $(top_srcdir)/share/qt/Info.plist #not installed
OSX_QT_TRANSLATIONS = da,de,es,hu,ru,uk,zh_CN,zh_TW
DIST_DOCS = $(wildcard doc/*.md) $(wildcard doc/release-notes/*.md)
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/bitcredit.ico \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_advantage_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_advantage_coverage.info test_advantage.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcredit.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Advantage-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings

APP_DIST_DIR = $(top_builddir)/dist
APP_DIST_EXTRAS = $(APP_DIST_DIR)/.background/$(OSX_BACKGROUND_IMAGE) $(APP_DIST_DIR)/.DS_Store $(APP_DIST_DIR)/Applications
OSX_BACKGROUND_IMAGE_DPIFILES := $(foreach dpi,$(OSX_BACKGROUND_IMAGE_DPIS),dpi$(dpi).$(OSX_BACKGROUND_IMAGE))
EXTRA_DIST = $(top_srcdir)/share/genbuild.sh qa/pull-tester/rpc-tests.sh qa/pull-tester/run-bitcredit-cli qa/rpc-tests $(DIST_DOCS) $(WINDOWS_PACKAGING) $(OSX_PACKAGING)
CLEANFILES = $(OSX_DMG) $(BITCREDIT_WIN_INSTALLER)
all: all-recursive
...
src/config/advantage-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/advantage-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/advantage-config.h
$(top_srcdir)/src/config/advantage-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/advantage-config.h src/config/stamp-h1
...
src/test/buildenv.py: $(top_builddir)/config.status $(top_srcdir)/src/test/buildenv.py.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
qa/pull-tester/run-bitcreditd-for-test.sh: $(top_builddir)/config.status $(top_srcdir)/qa/pull-tester/run-bitcreditd-for-test.sh.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
...
$(BITCREDIT_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(BITCREDITD_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(BITCREDIT_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(BITCREDIT_CLI_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/bitcredit.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/Advantage-Qt: $(BITCREDIT_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Advantage-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Advantage-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#deploy: $(OSX_DMG)
#deploy: $(BITCREDIT_WIN_INSTALLER)

$(BITCREDIT_QT_BIN): FORCE
	$(MAKE) -C src qt/$(@F)

$(BITCREDITD_BIN): FORCE
	$(MAKE) -C src $(@F)

$(BITCREDIT_CLI_BIN): FORCE
	$(MAKE) -C src $(@F)
...
#test_advantage.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_advantage -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_advantage_filtered.info: test_advantage.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_advantage_filtered.info
#	$(MKDIR_P) qa/tmp
#	-@TIMEOUT=15 qa/pull-tester/run-bitcreditd-for-test.sh $(JAVA) -jar $(JAVA_COMPARISON_TOOL) qa/tmp/compTool 0
#	$(LCOV) -c -d $(abs_builddir)/src --t BitcreditJBlockTest -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb
...
#test_advantage_coverage.info: baseline_filtered_combined.info test_advantage_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_advantage_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_advantage_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_advantage_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_advantage.coverage/.dirstamp:  test_advantage_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_advantage.coverage/.dirstamp total.coverage/.dirstamp

# Advantage: disable the outdated comparison tool test
# Ref https://github.com/bitcredit/bitcredit/issues/4545
#check-local:

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_advantage.coverage/ total.coverage/ $(OSX_APP)
